...
Among the four models evaluated in this report, the Artificial Neural Network achieved the best overall performance (87.42\% accuracy, Kappa 0.8322), closely followed by the Support Vector Machine with an RBF kernel (85.96\%, 0.8128). The optimized Decision Tree (78.17\%, 0.7089) and the Gaussian Naive Bayes classifier (68.17\%, 0.5756) trailed further behind. The lower performance of Naive Bayes is consistent with its strong feature-independence assumption, which does not hold for the highly correlated Sentinel-2 spectral bands. Across all models, the \textit{Vegetacion} and \textit{Zona\_urbana} classes were classified most reliably, while \textit{Suelo\_desnudo} and \textit{Cultivos} proved more challenging to distinguish spectrally due to their similar reflectance signatures.

The complete comparative analysis, including the KNN results, will be integrated once all team contributions are compiled.
